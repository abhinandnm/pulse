...
class RouteHealth(BaseModel):
    ...
    error_rate: float = Field(default=0.0, ge=0.0, le=1.0, description="Rolling error rate (0.0 to 1.0)")
    timeout_rate: float = Field(default=0.0, ge=0.0, le=1.0, description="Rolling timeout rate (0.0 to 1.0)")
    p95_latency_ms: float = Field(default=0.0, ge=0.0, description="95th percentile latency in milliseconds")
    total_transactions: int = Field(default=0, ge=0, description="Total sample size evaluated in current window")
    failed_transactions: int = Field(default=0, ge=0, description="Total failures in current window")
